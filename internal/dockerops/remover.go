...
func RemoveContainer(
	...
	clog.Info("Removing container", "containerID", containerID)
	...
	if err != nil {
		if strings.Contains(err.Error(), "No such container") {
			clog.Error("Notfound container", "containerID", containerID, "err", err)
			return
		} else {
			clog.Error("Failed to remove container", "containerID", containerID, "err", err)
	...
	if err != nil {
		clog.Error("Failed to update MySQL for container", "containerID", containerID, "err", err)
	} else {
		clog.Debug("MySQL updated for removed container", "containerID", containerID)
	}
